...
2. 添加任务（每天 09:00）：
```bash
0 9 * * * /root/scripts/send-daily-report.sh
```

3. 创建脚本 `/root/scripts/send-daily-report.sh`：
```bash
#!/bin/bash
# 调用 OpenClaw 发送日报
# 具体实现取决于你的消息推送方式
...
### 查看答题日志
```bash
ls -lt /root/.openclaw/workspace/memory/question-log/
```

### 查看待复习题目
```bash
grep "下次复习" /root/.openclaw/workspace/MEMORY.md
```
